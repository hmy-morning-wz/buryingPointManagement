      ...
      <div class="content">
        <div style="margin-top:20px;padding:24px;">
          ...
          >
            ...
            <span slot="channelName" slot-scope="channelName,record">
              <!-- <a-button type="link">{{productName}}</a-button> -->
              <span class="textSpan" @click="labelCheck(record)">{{channelName}}</span>
            </span>
            <span slot="action" slot-scope="action">
              <a @click="delClick(action)">删除</a>
              <a-divider type="vertical" />
              <a @click="updateClick(action)">编辑</a>
              <a-divider type="vertical" />
            ...
            <template>
              <a-form :form="form" :label-col="{ span: 8 }" :wrapper-col="{ span: 12 }">
                ...
                <a-form-item label="渠道标签：">
                  <a-input
                    v-decorator="['channelLabel', { rules: [{ required: true, message: '请输入渠道标签值!' }] }]"
                    :disabled="disable"
                    placeholder="请输入渠道标签值"
                  />
                </a-form-item>
...
<script>
import httpService from "../../service/ChannelManagement/httpService";
...
export default {
    ...
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          ellipsis: true,
          key: "channelLabel",
          scopedSlots: { customRender: "channelLabel" }
        },
        {
          title: "操作",
          key: "action",
  ...
  methods: {
    ...
    labelCheck(value) {
      ...
      this.detail.id = value.id;
      this.dialogVisible = true;
      this.dialogTitle = "查看渠道"
      this.routeName = "checkChannel";
      this.getChannelsList();
      this.getAppList();
      console.log("await==");
      this.getDetails(value.id);
    ...
    //   values.apps = apps;
      ...
      //   data.type = "page";
      if (this.routeName === "addChannel") {
        const data = this.handparams(values);
        httpService.logchannelconf(data).then(
          response => {
        ...
        const data = this.handparams(values);
        httpService.updatelogchannelconf(data).then(
          response => {
            //console.debug('response===',response.data)
            if (+response.code === 200 && response.data) {
              this.dialogVisible = false;
    ...
    getDetails(id) {
      ...
      httpService.getDetails(param).then(
        ...
        //   detail.channels = channels;
          ...
          //   this.form = detail;
          if (this.routeName === "checkChannel") {
            // this.limit = true;
            this.disable = true;
            this.dialogVisible = true;
          } else {
