    ...
    <el-scrollbar style="height:100%;">
      <template>
        <div>
          <el-row class="mr20">
            <el-col :span="24">
              <span class="float-left ml20 mt20 mb20">{{title}}</span>
            </el-col>
          </el-row>
        </div>
      </template>
...
export default {
  ...
  async created() {
    ...
    if (routeName === 'addEventAnalysis2') {
      // this.tableForm.goodsId = 5;
      this.title= '新建事件分析';
      this.getEventType(routeName)
       this.getEventConditionByProductId();
    } else if (
      routeName === 'checkEventAnalysis2' ||
      routeName === 'updateEventAnalysis2'
    ) {
      this.title= '编辑事件分析'
  ...
  methods: {
    ...
    getTableFormData() {
      ...
      this.tableForm.productId = this.productId
    },

    getEventConditionByProductId(){
      const data = this.productId;
      httpService.getEventConditionByProductId(data).then(
        response => {
         
          if (+response.code === 200 && response.data) {
            let array =  JSON.parse(response.data)
            this.tagsFilterOptions = JSON.parse(JSON.stringify(array))
          } else {
            // alert(response.msg || '新增任务失败')
            this.$message.error(
        ...
        httpService.logeventdeminconf(data).then(
          response => {
            ...
            if (+response.code === 200 && response.data) {
              ...
              this.$router.push({ path: "/productAnalysis",query:{id:this.productId +"",pageTitle:this.pageTitle,activeKey:"3"}});
    ...
    getDetails(routeName) {
      ...
      httpService.getDetails(param).then(
        response => {
          ...
          this.getDimenList()
          // this.getUserType();
          if (routeName === 'checkEventAnalysis2') {
            this.limit = true
            this.disable = true
          }
          this.checkTable()
